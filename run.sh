...
# /bin/sh as a lowest common denominator and there is no reliable way to
# invoke any possible implementation of it as a login shell from the ExecStart
# command. (Most shells offer a command-line option such as -l to make them
# behave like a login shell, but AFAIK this is not required by POSIX.) So
# we're left having to source the profile explicitly using a wrapper script.
#
